...
void iodata::validator::check_unknown_fields(record *p, const record_type *r)
{
    ...
    for (map<string, item *>::iterator p_it = p->x.begin(); p_it != p->x.end(); ++p_it) {
        bool found = false;
        for (vector<node *>::const_iterator r_it = r->nodes.begin();
             !found && r_it != r->nodes.end();
             ++r_it)
    ...
    assert(counter > 0);
    throw validator::exception((string) "unknown field" + (counter > 0 ? "s: " : ": ")
                               + unknown_fields);
}

void iodata::validator::check_fields(
    record *p, const record_type *r, bool add_defaults, item *items[], unsigned N)
{
    ...
    for (unsigned i = 0; i < N; ++i) {
        ...
        if (items[i] == NULL) // field is not present
        {
            ...
            } else if (add_defaults) // use the default value
            {
                if (not n->is_array) {
#if 0
#define _append(bit, type) \
    if ((n->flag & (bit | ARRAY)) == bit) \
        items[i] = p->x[n->name] = new type(n->type##_value);
#else
#define _append(bit, type) \
    if (n->is_##type() and not n->is_array) \
        items[i] = p->x[n->name] = new type(dynamic_cast<const node_##type *>(n)->value);
#endif
...
#undef _append
#if 0
#define _append_empty(bit, type) \
    if ((n->flag & (bit | ARRAY)) == bit) \
        items[i] = p->x[n->name] = new type;
#else
...
void iodata::validator::check_children(
    record * /*p*/, const record_type *r, bool write, item *items[], unsigned N)
{
    for (unsigned i = 0; i < N; ++i) {
        if (items[i] == NULL)
            continue;
        ...
        if (n->is_array) {
          ...
          switch(element_type)
          {
            ...
            case RECORD:  check_record(cast_and_check<record>(pa->x[j]), r->nodes[i]->type, write) ;
                    ...
                    if (n->is_record())
                        check_record(cast_and_check<record>(pa->x[j]),
                                     dynamic_cast<const node_record *>(n)->type,
                                     write);
#endif
                } catch (exception &e) {
            ...
            check_record(cast_and_check<record>(items[i]),
                         dynamic_cast<const node_record *>(n)->type,
                         write);
    }
}
    ...
    switch(f)
    {
      ...
      case BITMASK: flag = *cast_and_check<bitmask>(items[i])==r->nodes[i]->bitmask_value ; break ;
        ...
        if (n->is_array)
            ...
            flag = cast_and_check<integer>(items[i])->x
                   == dynamic_cast<const node_integer *>(n)->value;
        else if (n->is_bytes())
            flag = cast_and_check<bytes>(items[i])->x == dynamic_cast<const node_bytes *>(n)->value;
        else if (n->is_bitmask())
            flag = *cast_and_check<bitmask>(items[i])
                   == dynamic_cast<const node_bitmask *>(n)->value;
        else
...
void iodata::validator::link()
{
    for (map<string, record_type *>::iterator t = types.begin(); t != types.end(); ++t) {
        ...
        for (vector<node *>::iterator nn = t->second->nodes.begin(); nn != t->second->nodes.end();
             ++nn) {
            node *n = *nn;
...
iodata::record *iodata::validator::record_from_file(const char *path,
...
bool iodata::validator::record_to_file(const char *path,
                                       const char *record_type,
                                       iodata::record *data,
                                       string &serialized)
{
