...
struct node
{
    ...
    node(const string &n, bool a, bool m)
        : name(n)
        , is_array(a)
        , is_mandatory(m)
    {}
...
{
    int32_t value;
    node_integer(const string &n, bool a, bool m, int32_t v)
        : node(n, a, m)
        , value(v)
    {}
    ~node_integer() {}
...
struct node_record : public node
{
    string type_name;
    record_type *type;
    node_record(const string &n, bool a, bool m, const string &tn, record_type *t = NULL)
        : node(n, a, m)
        , type_name(tn)
    {
        type = t;
...
struct validator
{
    ...
    struct exception : public iodata::exception
    {
        ...
        string info() const
        {
            return (string) "iodata::validator::exception, " + message() + " at " + node_path;
        }
        ~exception() throw() {}
    };
    ...
    bool record_to_file(const char *path,
    ...
    void check_record_after_read(record *p, const string &type_name)
    {
        return check_record(p, type_by_name(type_name), false);
    }
    void check_record_before_write(record *p, const string &type_name)
    {
        return check_record(p, type_by_name(type_name), true);
    }
...
class output
{
    ...
    void print_chunk()
    {
        unsigned len = get_length();
        os.write(buffer + position, len);
        position += len;
    }
    void hard_reset();
    void soft_reset();
    ...
    {
        for (indent += indent_step; spaces.length() < indent; spaces += "           ")
            ;
    }
    void indent_decrement()
    {
    ...
    void comma_output(bool &flag, bool one)
    {
        if (flag) {
            if (one)
                os << ", ";
    ...
    void open(char ch, bool one)
    {
        if (one)
            os << " " << ch << " ";
        else
            newline_output(), indent_increment(), os << ch, newline_output();
    }
    void close(char ch, bool one)
    {
        if (one)
            os << " " << ch;
        else
            indent_decrement(), newline_output(), os << ch;
    }

    unsigned reserve_width(unsigned step)
    {
        unsigned pos = width.size();
        width.resize(pos + step);
        return pos;
    }
